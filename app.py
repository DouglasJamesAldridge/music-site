...
def init_db():
    ...
    """)
    # unsubscribe_token lets us identify a subscriber from a link
    # without exposing their email in the URL
    conn.execute("""
...
def get_or_create_token(email):
    """
    Returns an existing unsubscribe token for this email,
...
def send_email(to_email, subject, html_body):
    ...
    try:
        ...
        # Port 587 is the standard for Gmail SMTP with TLS
...
def build_email_html(link, unsubscribe_url):
    """
    Build the HTML email body with the music link and unsubscribe footer.
    """
    return f"""
    <div style="font-family: Georgia, serif; max-width: 540px; margin: 0 auto; padding: 2rem; background: #0d0d0d; color: #e8e8e8;">

      <p style="font-size: 0.75rem; letter-spacing: 0.15em; text-transform: uppercase; color: #c8ff00; margin-bottom: 1.5rem;">
        New Drop
      </p>

      <h1 style="font-size: 2rem; font-weight: normal; line-height: 1.2; margin-bottom: 1.5rem; color: #e8e8e8;">
        Something new just dropped.
      </h1>

      <p style="font-size: 1rem; color: #aaa; line-height: 1.7; margin-bottom: 2rem;">
        A new sketch is up. Click below to listen.
      </p>

      <a href="{link}"
         style="display: inline-block; background: #c8ff00; color: #0d0d0d; padding: 0.85rem 1.8rem;
                text-decoration: none; border-radius: 6px; font-weight: 700; font-family: sans-serif;
                font-size: 0.95rem;">
        Listen now
      </a>
      ...
      <p style="font-size: 0.75rem; color: #555; line-height: 1.6;">
        You're receiving this because you signed up for Sketches updates.<br/>
        <a href="{unsubscribe_url}" style="color: #555; text-decoration: underline;">
          Unsubscribe
        </a>
      </p>
...
def unsubscribe(token):
    """
    When a subscriber clicks their unsubscribe link, we look up their
    email from the token and remove them from all lists.
    """
    conn = get_db_connection()
    row = conn.execute(
...
def admin_send():
    """
    Called from the admin page — sends an email to all Sketches subscribers.
    ...
    # Fetch all subscribers on the chosen list
    conn = get_db_connection()
    subscribers = conn.execute(
...
# Run init_db when the module loads
init_db()
